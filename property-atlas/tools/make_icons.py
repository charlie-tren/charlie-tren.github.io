"""Rasterise pin.svg into the icon files, with the right alpha for each target.

WHY THIS EXISTS HERE. Until 10/09/2026 this page shipped ONE icon tag, an SVG, so
the tab was blank in Safari and older Chrome and iOS had nothing to mask. It went
unnoticed because Property Atlas was never added to tools/test_estate_head.mjs,
which is why that registration lands in the same commit as this file.

The source is pin.svg, unchanged: the mark is settled (08/09/2026, "stick with
current logo") and this only rasterises it.
...
- **Browser tabs** (favicon.ico, favicon-192.png, favicon-48.png) want the corners
  TRANSPARENT, so the rounded tile reads as a tile. These were previously flattened
  onto white, which put four white dots in the corners of every dark tab bar - and
  because every page without its own icon falls back to /favicon.ico, that was the
  whole estate.
