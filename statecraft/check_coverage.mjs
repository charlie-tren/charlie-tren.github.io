...
for (let i = 0; i < DRAWS; i += 1) {
  ...
  const start = data.countries[Math.floor(rnd() * data.countries.length)];
...
const never = data.countries.filter((c) => !winners.has(c.code)).map((c) => c.code);
const top = table[0];
...
for (const [code, n] of table) {
  const name = data.countries.find((c) => c.code === code).name;
  const bar = '#'.repeat(Math.round((n / played) * 120));
  console.log(`  ${code}  ${String(n).padStart(4)}  ${((n / played) * 100).toFixed(1).padStart(5)}%  ${bar} ${name}`);
}
console.log(`\nNever the answer: ${never.length ? never.join(', ') : 'none, all twenty can win'}`);
console.log(`Most common answer takes ${((top[1] / played) * 100).toFixed(1)}% of designs.`);
console.log(`Never the runner-up either: ${data.countries.filter((c) => !runnersUp.has(c.code)).map((c) => c.code).join(', ') || 'none'}`);
