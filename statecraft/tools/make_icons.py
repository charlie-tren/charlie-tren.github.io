"""Rasterise favicon.svg into the icon files, with the right alpha for each target.

WHY THIS EXISTS HERE. Statecraft went Live on 08/09/2026 carrying ONE icon tag, an
SVG, which is a blank tab in Safari and older Chrome and nothing for iOS to mask.
The estate head test passed it every run, because its icon assertion was
`icons.length > 0` and then checked the alpha of whatever it found - so one SVG
satisfied the whole check. Both halves are fixed in the same commit.

The source is favicon.svg, unchanged: this only rasterises the existing mark.
...
SVG_SQUARE = SVG.replace('rx="3"', 'rx="0"')
